module Facebook
  ...
  class Request
    class << self
      def parse_signed_request(signed_request, max_age=3600)
        ...
        raise 'Invalid request. (Invalid signature.)' \
          if base64_url_decode(encoded_sig) != OpenSSL::HMAC.hexdigest('sha256', ENV['FACEBOOK_SECRET'], encoded_envelope).split.pack('H*')
