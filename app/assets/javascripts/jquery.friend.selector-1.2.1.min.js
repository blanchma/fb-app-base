...
;(function(window, document, $, undefined) {
  ...
  _submit = function() {
    ...
    $('input.fs-friends:checked').each(function(){
      var name = $(this).parent().children('.fs-name').html();
      selected_friends= [parseInt($(this).val().split('-')[1], 10), name];
    });

    if ( fsOptions.facebookInvite === true ){

      var friends = selected_friends[0];
  ...
  _setFacebookFriends = function (k, v, predefined) {
    ...
    var link =  '<a class="fs-anchor" href="javascript://">' +
                  ...
                  //'<input class="fs-friends" type="hidden" name="friend[]" value="fs-'+v[k].id+'" />' +
                  '<img class="fs-thumb" src="https://graph.facebook.com/'+v[k].id+'/picture?width=60&height=60" />' +
  ...
  _initEvent = function() {
    ...
    wrap.delegate('.fs-anchor', 'click.fs', function(){
      _select($(this));
      _submit();
    });
  ...
  _find = function(t) {
    ...
    if ( search_text_base !== '' ) {
      ...
      var elements = $('#fs-user-list .fs-fullname');
    }
